...
## 6. 최근 업데이트 내역 (Release Notes)

### 🚀 v4.1.0 (2026.03.13)
*   **매집 의심 종목 표출 안정화:** 서버 스캔 데이터 동기화 이슈를 해결하여, 사용자가 설정한 연속 매수 기준일(예: 3일~5일)을 충족하는 조용한 매집 종목이 정상적으로 '나의 매집 의심 종목' 리스트에 노출되도록 개선했습니다.
*   **투자 심리(온도계) 로직 정교화:** 2,800여 개 전체 종목이 아닌, 자금이 집중되는 **170여 개 핵심 에이스 종목(관심종목, 주도 섹터주 등)**의 수급만 압축하여 계산하도록 변경했습니다. 잡음(노이즈)을 줄여 시장의 체감 매수 강도를 훨씬 민감하게 반영합니다.
    *   *로직:* 기본 50도 + (외인 매수/매도 방향 ±10도) + (기관 매수/매도 방향 ±10도) + (핵심 종목 외인+기관 합산 순매수 대금 / 200)
*   **UI 디테일 개선 (수급박스 모달):** 사용자 피드백을 반영하여 종목 상세(수급박스) 진입 시 상단에 표시되는 종목명과 현재가 폰트 크기를 약간 줄여(24 -> 22) 화면 답답함을 해소했습니다.
*   **비어있는 면 막대그래프(차트) 설명:** 면 막대그래프의 꽉 찬 면과 꽉 차지 않은 선은 "전일 거래량 대비 당일 거래량"을 나타내는 것으로, 꽉 채워지지 않았다면 아직 전일 거래량을 넘어서지 못했음을 시각적으로 직관적이게 보여줍니다.
